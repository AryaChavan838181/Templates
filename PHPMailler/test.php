...
function sendOtp($email) {
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'no.reply.01.coding@gmail.com';                    //SMTP username
        $mail->Password   = 'xmel tbor nybn fznj'; // Your app password (for Gmail)
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Or SMTPS if port 465
        $mail->Port       = 587; // Or 465

        $mail->setFrom('no.reply.01.coding@gmail.com', 'Your Name');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    exit; // Important: Stop execution after handling POST
}
?>
    ...
    <script>
        ...
        sendOtpBtn.addEventListener('click', () => {
            ...
            .then(data => {
                ...
                if (data.status) {
                    startResendTimer();
                    otpInputGroup.style.display = 'flex'; // Show OTP input and button
                    sendOtpBtn.style.display = 'none';
                }
            })
            .catch(error => {
